...
void backup(Dungeon &dun)
{
    ...
    /*save player*/
    //saveroom(dun.getGameroom(),outfile);
...
void saveplayer(Player *player,vector<Room> gameroom,ofstream &outfile)
{
    ...
    for(int i=0;i<gameroom.size();i++)
    {
        ...
        //outfile<<1;
        outfile<<endl;
        //cout<<"finish positiopn"<<endl;
        //
        //cout<<"the gameroom objece number"<<gameroom[i].objects.size()<<endl;
        for(int j=0;j<gameroom[i].objects.size();j++)
        {
            ...
            else if(tagofobj=="CHEST")
            {
                ...
                //cout<<"name"<<chestptr->getName()<<endl;
...
{
    //cout<<"now setting the room object"<<endl;
    ...
    for(int i=0;i<numberofobj;i++)
    {
        ...
        if(objtag=="NPC")
        {
            ...
            for(int i=0;i<numberofinventory;i++)
            {
                ...
                int att,def,health;
                infile>>itemname>>type>>health>>att>>def;
                //cout<<itemname<<" "<<type<<" "<<health<<" "<<att<<" "<<def<<endl;
                Item it(itemname,type,health,att,def);
        ...
        if(objtag=="CHEST")
        {
            ...
            //cout<<"chest_name:"<<chestptr->getName();
            objectlist.push_back(chestptr);
            //cout<<objectlist[objectlist.size()-1]->getName()<<endl;
        }
        if(objtag=="Monster")
        {
