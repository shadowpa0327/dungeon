...
{
    //cout<<"calling the copy constructor of dungeon"<<endl;
...
{
    //readthefuckingfile(player,gameroom,exitroom,numberof_collected_item);
...
Dungeon::Dungeon(){
    ...
    gameroom=setmap();//�]�w�a��
    exitroom=setting_roominfor(gameroom);//�]�w�a�����Y

    cout<<"the exit room is room"<<exitroom->getIndex()<<endl;
    Player *playerptr = new Player(playername , 100 , 100 ,100);
    playerptr->setCurrentRoom(&gameroom[0]);//0���Ь��J�f
    this->player = playerptr ;

    cout<<"please input the number of the item that are going to collect"<<endl;
    numberof_collected_item=getnumberfrom_ifstream();//�Ψ�Ʀs��tool�������*/

    //cin>>numberof_collected_item;
    //this->room = &gameroom[0] ;
};
...
bool Dungeon::task_findexit()
{
    ...
    if(player->getCurrentRoom()==exitroom){
        //if(idx==0)cout<<"Congradulation you found the exit!!"<<endl;
        //idx++;
        return true;
    }
    else{
...
bool Dungeon::task_collectitem()
{
    if(player->countitem()>=numberof_collected_item){
        //cout<<"Congradulation you successfully collect the desired number of item!!"<<endl;
        return true;
    }
    else{
        return false;
